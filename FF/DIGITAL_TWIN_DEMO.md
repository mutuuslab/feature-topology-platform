...
문서 원문을 추출해 둔 정적 자산도 저장소·배포본에 없다(`public/spec` 삭제 — `/spec/data/**` · `/spec/screens/**` · `/spec/arch/**` 는 정적 파일로 404).
...
- **배포본 동일성**: 같은 하네스를 배포 URL(`https://feature-topology.pages.dev` — `VEH_BASE` · `VEH_PREFIX` 로 대상/접두사 교체)에 그대로 돌려 **5개 상태 전부를 개발 서버와 비교 실측**했다.

  | 상태 | carPx (dev) | carPx (prod) | carMean (dev → prod) | 휘도>90 | 채도>20 % |
  |---|---:|---:|---|---:|---:|
  | 1 실 모델 · HDRI | 144,227 | **144,227** | 110.0 → 110.0 | 41.3 % | 43.1 % |
  | 2 X-ray 차체 | 144,025 | 144,025 | 106.0 → 105.9 | 38.0 % | 43.3 % |
  | 3 관절(Joint) | 146,707 | 146,698 | 107.7 → 107.7 | 40.0 % | 42.7 % |
  | 4 절차적 환경 | 275,076 | 275,034 | 65.7 → 65.7 | 20.9 % | 40.4 % |
  | 5 절차적 셸 + X-ray | 274,960 | 274,961 | 65.7 → 65.7 | 20.4 % | 39.8 % |

  1번은 **SHA-256 까지 동일**(byte-identical), 나머지는 최대 42 px(0.03 % 이하) 차이로 이는 프레임 캡처 시점의 바퀴 회전 위상(`simTimeMs`) 차이다.
  즉 이 렌더는 CDN 배포 여부가 아니라 결정적 입력(자산 + 카메라 프리셋 + `simTimeMs`)의 함수다.
