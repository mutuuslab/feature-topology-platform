...
const DEFS: PlaneDef[] = [
  {
    ...
    demos: [
      ...
      {
        ko: '의사결정 (원천 · 정책)',
        en: 'Decisions (Source & Policy)',
        routes: ['/impact', '/spec/experiment', '/spec/conflict', '/spec/exception'],
      },
    ],
  },
  {
    ...
    demos: [
      {
        ko: '검증',
        en: 'Verification',
        routes: ['/verify/evidence', '/readiness/FEAT-BDC-001', '/spec/compliance', '/spec/scenario'],
      },
      {
        ko: '의사결정 (기술 적합성)',
        en: 'Decisions (Technical Fitness)',
        routes: ['/decisions/verification', '/decisions/supplier', '/decisions/report'],
      },
    ],
  },
  {
    ...
    demos: [
      { ko: '내 업무 · 홈', en: 'My Work & Home', routes: ['/', '/home/customize', '/login', '/onboarding'] },
      {
        ko: '배포 · 운영',
        en: 'Deploy & Ops',
        routes: ['/ops/FEAT-BDC-001', '/ops/campaign', '/ops/policy', '/ops/telemetry', '/ops/incident', '/spec/cicd'],
      },
      {
        ko: '연동 · 과금',
        en: 'Integration & Billing',
        routes: ['/integration/connectors', '/integration/sync', '/spec/billing'],
      },
      {
        ko: '분석 · 감사',
        en: 'Insights & Audit',
        routes: ['/insights/reports', '/cost', '/insights/audit', '/insights/glossary', '/spec/business'],
      },
      {
        ko: '관리자 · 보안',
        en: 'Admin & Security',
      ...
      {
        ko: '의사결정 (허가 · 명령)',
        en: 'Decisions (Authorization)',
        routes: ['/decisions/center', '/decisions/deploy'],
      },
    ],
  },
  {
    ...
    demos: [
      ...
      {
        ko: '대상 · 활성화',
        en: 'Targets & Activation',
        routes: ['/fleet', '/activation', '/ops/runtime', '/variants/FEAT-BDC-001'],
      },
    ],
  },
];
...
export const SPEC_PLANE_NAV: SpecPlaneNav[] = [
  ...
  // 공유 기반 — 기준 화면을 소유하지 않는다(다섯 번째 Plane이 아니다). 공유 참조 문서만 둔다.
  {
    ...
    screens: [],
    screenGroups: [],
    demos: [{ ko: '기준 참조', en: 'Reference', routes: ['/arch', '/ui', '/spec/changelog', '/spec/glossary'] }],
    shared: true,
  },
];
